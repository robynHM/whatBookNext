...
trait BaseSpecWithApplication extends BaseSpec with GuiceOneServerPerSuite with ScalaFutures with BeforeAndAfterEach with BeforeAndAfterAll with Eventually {
  ...
  lazy val service: BookService = injector.instanceOf[BookService]
  lazy val serviceLayer: ApplicationService = injector.instanceOf[ApplicationService]
  ...
  override def fakeApplication(): Application =
    new GuiceApplicationBuilder()
      .configure(Map(
        "mongodb.uri"                                    -> "mongodb://localhost:27017/whatBookNext"
      ))
      .build()
